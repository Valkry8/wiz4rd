...
clear;clear;clear
  ...
  # // Banner
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo -e "  Welcome To GONDRONG ${YELLOW}(${NC}${green} Stable Edition ${NC}${YELLOW})${NC}"
...
MYIPV6=$(curl -sS ipv6.icanhazip.com)
...
# USERNAME
rm -f /usr/bin/user
username=$(curl https://raw.githubusercontent.com/Valkry8/Regist/MONSTER/afk | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
expx=$(curl https://raw.githubusercontent.com/Valkry8/Regist/MONSTER/afk | grep $MYIP | awk '{print $3}')
echo "$expx" >/usr/bin/e
# DETAIL ORDER
...
Exp1=$(curl https://raw.githubusercontent.com/Valkry8/Regist/MONSTER/afk | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
print_install "Membuat direktori xray"
    mkdir -p /etc/xray
    IPV4=$(curl -sS ifconfig.me)
    IPV6=$(curl -sS --connect-timeout 3 ipv6.icanhazip.com)
    echo "$IPV4" > /etc/xray/ipvps
    [ -n "$IPV6" ] && echo "$IPV6" >> /etc/xray/ipvps
...
function first_setup(){
    timedatectl set-timezone Asia/Gorontalo
...
restart_system() {
    USRSC=$(wget -qO- https://raw.githubusercontent.com/Valkry8/Regist/MONSTER/afk | grep $ipsaya | awk '{print $2}')
    EXPSC=$(wget -qO- https://raw.githubusercontent.com/Valkry8/Regist/MONSTER/afk | grep $ipsaya | awk '{print $3}')
...
clear
    print_install "Core Xray Terbaru"
    # install xray
...
latest_version="$(curl -s https://api.github.com/repos/XTLS/Xray-core/releases/latest | grep -o '"tag_name":"[^"]*"' | sed -E 's/.*"v([^"]+)".*/\1/')"
bash -c "$(curl -L https://github.com/XTLS/Xray-install/raw/main/install-release.sh)" @ install -u www-data --version "v$latest_version"
    ...
    print_success "Core Xray v$latest_version Latest Version"
...
/etc/init.d/cron restart
    ...
    systemctl enable --now rc-local98
