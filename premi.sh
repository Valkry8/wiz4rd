...
# USERNAME
rm -f /usr/bin/user
username=$(curl https://raw.githubusercontent.com/prem-sc/Regist/MONSTER/afk | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
expx=$(curl https://raw.githubusercontent.com/prem-sc/Regist/MONSTER/afk | grep $MYIP | awk '{print $3}')
echo "$expx" >/usr/bin/e
# DETAIL ORDER
...
Exp1=$(curl https://raw.githubusercontent.com/randi-bug/Regist/MONSTER/afk | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
clear
# REPO    
    REPO="https://raw.githubusercontent.com/prem-sc/wiz4rd/MONSTER/"
